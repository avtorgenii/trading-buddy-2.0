<script>
	...
	async function processPositionData() {
		...
		try {
			...
			positionSize = positionSize || parseFloat(results.volume);
	...
	<div
      ...
      "
		...
		<div class="bg-zinc-800 rounded-t-2xl flex items-center justify-center p-1 mt-3">
			<div class="w-full h-80 sm:h-80 md:h-96 lg:h-96 xl:h-106 ">
				{#key selectedTicker}
				...
				<div class="flex items-center gap-2 mb-4 flex-wrap">
					...
					<div class="flex-1 flex gap-2 min-w-[200px]">
						<input
							...
							bind:value={tp.price}
							placeholder="Price"
							class="bg-zinc-800 flex-1 min-w-0 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-base"
						/>
					</div>
